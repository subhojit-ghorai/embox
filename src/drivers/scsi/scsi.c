...
static int scsi_send_inquiry(struct scsi_dev *dev) {
	...
	cmd.scmd_opcode = SCSI_CMD_OPCODE_INQUIRY,
	cmd.scmd_len = sizeof(struct scsi_cmd_inquiry),
...
static int scsi_send_sense(struct scsi_dev *dev) {
	...
	if (!(acode == 0x28 || acode == 0x29)) {
		log_erro("Don't know how to recover unknown error %x", acode);
		return -1;
	}
	return 0;
}
...
int scsi_dev_attached(struct scsi_dev *dev) {
	...
	dev->state = SCSI_STATE_ATTACHED;

	if (ret = scsi_send_inquiry(dev)) {
		log_debug("INQUIRY failed");
		return -1;
	}

state_test_unit:
	if (ret = scsi_send_test_unit(dev)) {
state_sense:
		if (ret = scsi_send_sense(dev)) {
			goto state_test_unit;
		}
	}

	ret = scsi_send_capacity10(dev, &scsi_state_capacity10);
	if (ret) {
		goto state_sense;
	}
